...
#	Output a file of sequences which weren't added due to being duplicates - headers consist of concatenated headers from all duplicates
#		This is so that duplicates can be thoroughly assessed afterwards
# To run: 
...
def sequence_cleaner(fasta_file, max_length=0, min_length=0, por_nucl=0):
    ...
    for seq_record in SeqIO.parse(fasta_file, "fasta"):
       ...
       # If it is already in the hash table, we're just going to add to dupl and concatenate its header to the original seq in hash table
            else:
                dupl[sequence] = seq_record.description
                sequences[sequence] += "_" + seq_record.description
       #any that fail parametes go into QC fail dictionary
        else:
